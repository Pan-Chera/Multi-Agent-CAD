...
Edit [multi_agent_cad/config.py(config.py):
...
For OpenAI, edit [config.py(config.py):
...
| Config editing | edit [config.py(config.py) | fill a form |
...
Or bypass via code: set `force_refresh: True` in `get_default_initial_state` in [multi_agent_cad/graph.py(graph.py).

### Customizing the prompt

Edit `USER_REQUEST` in [multi_agent_cad/config.py(config.py), e.g.:
