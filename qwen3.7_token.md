...
- 主体为矩形长方体
- 四个垂直通孔（沿 Z 方向贯穿整个块的高度）
- 通孔呈 2×2 对称阵列排布
- 通孔位置关于 X 轴和 Y 轴双向对称
- 倒角仅施加在顶面周边外轮廓棱边
- 倒角不施加在通孔内壁（孔口保持尖锐）、底面或侧面

| 类别 | 完成率 |input | cache_w | cache_r | output | total | API 调用 | 费用（元） |
|---|---|---:|---:|---:|---:|---:|---:|---:|
...
- 轴线沿 X 方向延伸
- 三段同轴圆柱段（左、中、右）
- 中间段直径最大，两端段直径较小（中粗两端细）
- 左右两端段直径相等（关于中段对称）
- 左右两端面边均有倒角
- 矩形键槽位于中段（最粗部分）顶部
- 键槽沿 X 方向延伸（不贯穿全长）
- 键槽位于顶部（不贯穿直径）
- 键槽有固定 Y 方向宽度和 Z 方向深度
...
- 支柱位置关于 X 轴和 Y 轴双向对称
- 每个支柱中心有一个同轴盲孔
- 盲孔不穿透底板（深度小于支柱+底板总厚度）
- 四个外部垂直角（盒体外侧四个棱）有圆角
- 内部垂直角、顶部边缘及支柱无圆角

| 类别 | 完成率 |input | cache_w | cache_r | output | total | API 调用 | 费用（元） |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| cad skill | 11/12(第11条不满足) | 199,344 | 0 | 1,961,344 | 27,957 | 2,188,645 | 53 | **2.88** |
...
- 单一实体
- 整体为 U 形支架（clevis）
- 整体关于 XZ 平面对称（Y 方向镜像对称）
- 两个垂直 U 形耳（clevis lugs）立于底板顶部
- 两个耳位于底板中心附近（非边缘）
- 两个耳之间有 Y 方向间隙（gap）
- 两个耳顶端呈半圆形轮廓（侧视图）
- 水平通孔贯穿两个耳（沿 Y 方向）
...
| 类别 | 完成率 |input | cache_w | cache_r | output | total | API 调用 | 费用（元） |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| cad skill | 16/18(第16、18条不满足) | 677,783 | 0 | 12,261,248 | 210,211 | 13,149,242 | 171 | **15.21** |
...
- 翅片外径大于桶外径
- 翅片为薄圆盘
- 底部有加厚法兰（base flange）与桶同轴
- 法兰外径大于翅片外径
- 法兰上有 6 个垂直安装孔贯穿法兰
- 安装孔在圆周上等角度分布（6 重旋转对称）
- 顶部有顶盖圆柱（top cap），直径大于桶直径，小于翅片外径
- 凸台从顶盖侧面伸出
- 凸台轴线向上倾斜（与水平面成 35° 角）
- 凸台通孔贯穿凸台全长
- 翅片外缘有小圆角
...
- 单一实体
- 整体为离心式叶轮
- 扁圆柱形背板盘（backplate）为底
- 中心轮毂圆柱立于背板顶部
- 轮毂与背板同轴
- 垂直通孔贯穿整个零件（从背板底到轮毂顶）
- 12 片相同的后弯式叶片
- 叶片立于背板顶部
- 叶片围绕轮毂等角度分布（12 重旋转对称）
- 每片叶片从内半径（接近轮毂）到外半径延伸
- 叶片顶部向后倾斜（与旋转方向相反）
- 叶片有固定厚度和高度
- 叶片根部有圆角（与背板和轮毂交界处）
- 背板顶部外圆边缘有圆角
...
| multi agent | 14/15(第13条不满足) | 80,922 | 0 | 0 | 39,733 | 120,655 | 7 | **1.20** |
...
| multi agent | 17/16(修复了第8条) | 71,493 | 0 | 10,496 | 56,503 | 138,492 | 6 | **1.45** |
...
**几何特征**：
- 多个独立实体（非单一实体，5 类零件各自独立）
- 整体为行星齿轮组件
- 平放在 XY 平面
- 包含 5 类零件：太阳齿轮、3 个行星齿轮、内齿圈、行星架板、3 根行星销轴
- 太阳齿轮位于中心
- 太阳齿轮为外齿（外圆周有齿）
- 3 个行星齿轮
- 行星齿轮为外齿
- 行星齿轮在圆周上等角度分布（3 重旋转对称，120° 间隔）
- 行星齿轮位于太阳齿轮与内齿圈之间
- 内齿圈与太阳齿轮同轴
- 内齿圈为内齿（内圆周有齿）
- 内齿圈位于最外圈
